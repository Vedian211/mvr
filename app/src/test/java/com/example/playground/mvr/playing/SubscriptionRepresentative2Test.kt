package com.example.playground.mvr.playing
...
class SubscriptionRepresentative2Test {
    ...
    @Test
    fun `When init and start getting update then ui state is initial and callback invoked`() = with(TestObjects()) {
        // When
    ...
    @Test
    fun `Given representative is initialized when subscribe then verify ui state`() = with(TestObjects()) {
        // When
    ...
    @Test
    fun `Given subscribed state when finish then navigate to Dashboard and clear resources`() = with(TestObjects()) {
        // When
    ...
    @Test
    fun `When re-init after restore then verify first open and subscription update happened once`() = with(TestObjects()) {
        // When
        representative.init(saveAndRestore)
        representative.startGettingUpdates(callback)
    ...
    @Test
    fun `Given death happened on loading step when re-init then death handled and state Empty state restored`() = with(TestObjects()) {
        // When
        representative.init(saveAndRestore)
        representative.startGettingUpdates(callback)
    ...
    @Test
    fun `Given death happened on Success step when re-init then death handled and Success state restored`() = with(TestObjects()) {
        // When
        representative.init(saveAndRestore)
        representative.startGettingUpdates(callback)
    ...
    @Test
    fun `Given death happened after success when re-init then death handled and any updates not called`() = with(TestObjects()) {
        // When
        representative.init(saveAndRestore)
        representative.startGettingUpdates(callback)
